...
export class FeatureService<TFeature extends Feature, TView extends View> {
  ...
  isAnyEnabled(view: TView | null = null) {
    var features
    if (view == null) {
      features = Object.keys(this.featureSignals) as TFeature[];
    } else {
      features = this.views[view].features;
    }
    return features.some(feature => this.featureSignals[feature as TFeature].enabled());
  }
